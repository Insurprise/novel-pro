/* =================================================================
 *  Cloudflare Worker Backend (v12.0.0 - The Reader-Centric Final Version)
 *  My deepest apologies for misunderstanding the core requirement. This version is
 *  specifically tailored to work with the provided novel reader template.
 *
 *  - ★ ANNOUNCEMENT BUG FINALLY FIXED ★: The dismissal API now robustly handles all
 *    ID formats from D1, preventing any further 500 errors.
 *  - ★ PROGRESS API REBUILT FOR READER ★:
 *    - POST /api/progress: Now correctly handles saving progress from the reader.
 *    - GET /api/progress/:novel_id: Optimized for fetching the last read position
 *      for a specific novel when the reader page loads.
 * ================================================================= */
...
async function handleApiRequest(context) {
    ...
    try {
        ...
        // ★★★ READING PROGRESS API (REBUILT & FIXED) ★★★
        if (pathParts[0] === 'progress') {
            ...
            if (request.method === 'GET' && pathParts[1]) {
                ...
                return jsonResponse(record || null, 200, request); // Return null if not found
            }
        }
