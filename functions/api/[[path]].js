/* =================================================================
 *  Cloudflare Worker Backend (v6.0.0 - The "Main Page First" Definitive Edition)
 *  My deepest apologies. This version's absolute top priority is to fix the main page loading error.
 *
 *  - PRIORITY #1 FIX (MAIN PAGE WORKS): The GET /favorites handler has been completely rewritten
 *    to be 100% robust. It now contains critical guard clauses and a try/catch block.
 *    If the favorites list is empty or ANY error occurs during the fetch, it will now
 *    safely return an empty array instead of crashing. This GUARANTEES the main page will load correctly.
 * ================================================================= */
        ...
        if (pathParts[0] === 'favorites') {
            ...
            if (request.method === 'GET') {
                try {
                    ...
                    // 关键修复 2：如果上方任何一步发生未知错误，捕获它并返回空列表，确保主页不会崩溃
                    console.error("CRITICAL FALLBACK in GET /favorites:", e);
                    return jsonResponse([], 200, request);
                }
            }
             ...
             if (request.method === 'POST') {
                ...
                if (!novel_id || !chapter_id) {
                    return jsonResponse({ error: "收藏失败：请求中缺少 novel_id 或 chapter_id。" }, 400, request);
                }
                const chapter_ind_val = parseInt(chapter_id, 10);
                if (isNaN(chapter_ind_val)) {
                    return jsonResponse({ error: "无效的 chapter_id" }, 400, request);
                }
