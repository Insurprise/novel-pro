/* =================================================================
 *  Cloudflare Worker Backend (v17.0.0 - The Definitive Schema-Fix Edition)
 *  My deepest apologies. This version fixes all 500 errors caused by critical
 *  discrepancies between the code and the actual database schema.
 *
 *  - ★ CRITICAL SCHEMA FIX ★: Removed the non-existent 'created_at' column
 *    from all 'FavoriteChapters' table queries (GET, POST).
 *  - ★ CRITICAL SCHEMA FIX ★: Removed the non-existent 'author' and 'description'
 *    columns from the 'Sites' table UPDATE query.
...
async function handleApiRequest(context){
    ...
    try {
        ...
        // ★★★ SITES API (PUT 500 ERROR FIXED) ★★★
        if(pathParts[0]==='sites'){
            if(request.method==='GET'){const t=url.searchParams.get('type');const{results}=await env.DB.prepare(`SELECT * FROM Sites ${t?'WHERE type = ?':''} ORDER BY name`).bind(...(t?[t]:[])).all();return jsonResponse(results,200,request)}
            if(user.role!=='admin')return jsonResponse({error:'无权操作'},403,request);
            if(request.method==='POST'){const d=await request.json();await env.DB.prepare("INSERT INTO Sites(name,subdomain,type)VALUES(?,?,?)").bind(d.name,d.subdomain,d.type).run();return jsonResponse({message:'创建成功'},201,request)}
            // ★ FIX: Removed non-existent 'author' and 'description' columns from the UPDATE query
            if(request.method==='PUT'&&pathParts[1]){const d=await request.json();await env.DB.prepare("UPDATE Sites SET name=?,subdomain=?,type=? WHERE id=?").bind(d.name,d.subdomain,d.type,pathParts[1]).run();return jsonResponse({message:'更新成功'},200,request)}
        ...
        // ★★★ FAVORITES API (GET & POST 500 ERRORS FIXED) ★★★
        if(pathParts[0]==='favorites'){
            // ★ FIX: Removed non-existent 'created_at' column from SELECT
            if(request.method==='GET'){const q=`SELECT f.novel_id,f.chapter_id,f.chapter_ind,f.chapter_title,s.name as novel_name FROM FavoriteChapters f JOIN Sites s ON f.novel_id=s.subdomain WHERE f.user_id=? ORDER BY f.id DESC`;const{results}=await env.DB.prepare(q).bind(userId).all();return jsonResponse(results,200,request)}
            // ★ FIX: Removed non-existent 'created_at' column from INSERT
        ...
        // [STABLE APIS: Announcements, Progress, Users]
